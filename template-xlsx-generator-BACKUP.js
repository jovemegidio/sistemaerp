...
// Baseado no arquivo ordem_completa_segura.js
...
class TemplateXLSXGenerator {
    ...
    async generateWithDirectFill(templatePath, outputPath, dadosOrdem) {
        ...
        try {
            ...
            console.log('✅ Template copiado como base');
            ...
            console.log(`✅ Dados aplicados DIRETAMENTE (${stats.size} bytes)`);
    ...
    async generateWithSafeExcelJS(templatePath, outputPath, dadosOrdem) {
        ...
        try {
            ...
            // 4. Aplicar dados de forma conservadora
            await this.aplicarMapeamentoConservador(worksheet, dadosOrdem);
            ...
            if (reductionPercent > 95) {
                ...
                console.log(`✅ Dados aplicados com sucesso (redução: ${reductionPercent.toFixed(1)}%)`);
    ...
    async aplicarMapeamentoConservador(worksheet, dadosOrdem) {
        ...
        try {
            ...
            // Aplicar dados da transportadora
            console.log('\n🚚 DADOS DA TRANSPORTADORA:');
            this.preencherCelulaSegura(worksheet, 'C13', dadosOrdem.transportadora, 'Transportadora');
            this.preencherCelulaSegura(worksheet, 'G13', dadosOrdem.contato_transportadora, 'Contato Transportadora');
            this.preencherCelulaSegura(worksheet, 'C14', dadosOrdem.fone_transportadora, 'Telefone Transportadora');
            this.preencherCelulaSegura(worksheet, 'G14', dadosOrdem.email_transportadora, 'Email Transportadora');
            
            // CPF/CNPJ da transportadora
            const cpfCnpjTransportadora = dadosOrdem.transportadora_cpf_cnpj || '';
            if (cpfCnpjTransportadora) {
                ['C15', 'D15', 'E15', 'F15'].forEach(cellAddr => {
                    this.preencherCelulaSegura(worksheet, cellAddr, cpfCnpjTransportadora, '', '@');
                });
            }
            
            // Email NFe
            this.preencherCelulaSegura(worksheet, 'G15', dadosOrdem.transportadora_email_nfe, 'Email NFe');
            this.preencherCelulaSegura(worksheet, 'H15', dadosOrdem.transportadora_email_nfe, '');
            ...
            console.log('⚠️ Erro na aplicação conservadora:', error.message);
            throw error;
        }
    }
    ...
    async generateWithTemplateCopyOnly(templatePath, outputPath, dadosOrdem) {
        ...
        if (templateExists) {
            ...
            await fs.promises.copyFile(templatePath, outputPath);
            console.log('✅ Template copiado preservando 100% da formatação original');

            const stats = await fs.promises.stat(outputPath);
            
            // Log dos dados que seriam aplicados (para debug)
    ...
    async generateWithExcelJS(ExcelJS, templatePath, outputPath, dadosOrdem) {
        ...
        try {
            ...
            const worksheet = workbook.worksheets[0];
            console.log('✅ Template copiado e carregado para preenchimento');
    ...
    async generateWithTemplateCopy(templatePath, outputPath, dadosOrdem) {
        ...
        if (templateExists) {
            ...
            await fs.promises.copyFile(templatePath, outputPath);
            console.log('✅ Template copiado como base Excel válida');
    ...
    async aplicarMapeamentoCompleto(worksheet, dadosOrdem) {
        ...
        // === TRANSPORTADORA ===
        console.log('\n🟡 TRANSPORTADORA:');
        this.preencherCelulaSegura(worksheet, 'C12', dadosOrdem.transportadora_nome || dadosOrdem.transportadora, 'Nome Transportadora');
        this.preencherCelulaSegura(worksheet, 'D12', dadosOrdem.transportadora_nome || dadosOrdem.transportadora, '');
        this.preencherCelulaSegura(worksheet, 'E12', dadosOrdem.transportadora_nome || dadosOrdem.transportadora, '');
        // Preencher G12 e campo amarelo com o telefone do modal
        try {
            worksheet.getCell('G12').value = dadosOrdem.transportadora_fone || dadosOrdem.fone_transportadora;
            worksheet.getCell('H12').value = dadosOrdem.transportadora_fone || dadosOrdem.fone_transportadora;
        } catch (e) { /* ignorar */ }
        
        this.preencherCelulaSegura(worksheet, 'C13', dadosOrdem.transportadora_cep, 'CEP');
        this.preencherCelulaSegura(worksheet, 'D13', dadosOrdem.transportadora_cep, '');
        this.preencherCelulaSegura(worksheet, 'F13', dadosOrdem.transportadora_endereco, 'Endereço');
        this.preencherCelulaSegura(worksheet, 'G13', dadosOrdem.transportadora_endereco, '');
        this.preencherCelulaSegura(worksheet, 'H13', dadosOrdem.transportadora_endereco, '');
        this.preencherCelulaSegura(worksheet, 'I13', dadosOrdem.transportadora_endereco, '');
        ...
        if (dadosOrdem.transportadora_cpf_cnpj) {
            ['C15', 'D15'].forEach(cellAddr => {
                try {
                    const cell = worksheet.getCell(cellAddr);
                    cell.value = dadosOrdem.transportadora_cpf_cnpj;
                    cell.numFmt = '@';
                    console.log(`   ✅ CPF/CNPJ: ${cellAddr} = ${dadosOrdem.transportadora_cpf_cnpj}`);
                } catch (e) { /* ignorar */ }
            });
        }
    ...
    preencherCelulaSegura(worksheet, cellAddress, value, label = '', numFormat = null) {
        try {
            ...
            if (value !== null && value !== undefined && value !== '') {
                ...
                // Aplicar formato numérico se especificado
                if (numFormat) {
